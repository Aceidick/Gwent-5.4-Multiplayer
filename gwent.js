...
class ControllerAI {
    ...
    async decoy(card, max, data) {
        ...
        if (targ) {
            ...
            // mirroring the row restriction enforced for the human player.
            // The scan is bounded so a target that is not on an own row (moved,
            // stale data) falls through to the default placement instead of
            // reading past the end of board.row.
    ...
    makeBank(faction, deck) {
        ...
        let cards = Object.keys(card_dict).map(cid => ({
            ...
            p => (([faction, "neutral", "weather", "special"].includes(p.card.deck) ||
                (faction === "old_realms" && p.card.deck === "realms") ||
    ...
    deckFromJSON(json, parse) {
        ...
        let cards = deck.cards.filter(c => {
            ...
            if (!([deck.faction, "neutral", "special", "weather"].includes(card.deck) ||
                (deck.faction === "old_realms" && card.deck === "realms") ||
